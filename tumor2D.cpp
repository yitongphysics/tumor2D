...
void tumor2D::stickyTumorInterfaceForces(){
	// local variables
    int ci, cj, ck, gi, gj, gk, vi, vj, vk, bi, bj, pi, pj, pk, boxid, sbtmp;
    double sij, rij, dx, dx_ab, dx_pa, dx_pb, dy, dy_ab, dy_pa, dy_pb, rho0, xi, yi, ri;
    double ftmp, fx, fy;
    double ntmp, nx, ny;
    double dd;
	vector<int> ztt(NVTOT,0);
...
void tumor2D::verticeBondRepulsions() {
    // local variables
    int ci, cj, ck, gi, gj, gk, vi, vj, vk, bi, bj, pi, pj, pk, boxid, sbtmp;
    double sij, rij, dx, dx_ab, dx_pa, dx_pb, dy, dy_ab, dy_pa, dy_pb, rho0, xi, yi, ri;
    double ftmp, fx, fy;
    ...
    for (bi = 0; bi < NBX; bi++) {
        ...
        while (pi > 0) {
            ...
            while (pj > 0) {
                ...
                //add repulsive vertice-bond interactions to prevent penetrating: now we have gi---P, gj---A, and gk=ip1[gj]---B
                gk = ip1[gj];
                
                //vectors: AP, BP and BA
                dx_pa = x[NDIM * gi] - x[NDIM * gj];
                dx_pb = x[NDIM * gi] - x[NDIM * gk];
                dx_ab = x[NDIM * gj] - x[NDIM * gk];
                if (pbc[0]){
                    dx_pa -= L[0] * round(dx_pa / L[0]);
                    dx_pb -= L[0] * round(dx_pb / L[0]);
                    dx_ab -= L[0] * round(dx_ab / L[0]);
                }
                //cout << dx_ab << endl;
                dy_pa = x[NDIM * gi + 1] - x[NDIM * gj + 1];
                dy_pb = x[NDIM * gi + 1] - x[NDIM * gk + 1];
                dy_ab = x[NDIM * gj + 1] - x[NDIM * gk + 1];
                if (pbc[1]){
                    dy_pa -= L[1] * round(dy_pa / L[1]);
                    dy_pb -= L[1] * round(dy_pb / L[1]);
                    dy_ab -= L[1] * round(dy_ab / L[1]);
                }
                
                // if possible to penetrate
                if (dx_pa*dx_pb+dy_pa*dy_pb<0) {
                    //calculate normal vector n: normalized and pointing outwards of dp
                    ntmp = dy_ab*dy_ab + dx_ab * dx_ab;
                    nx = dy_ab/ntmp;
                    ny =-dx_ab/ntmp;
                    if (dx_pa * nx + dy_pa + ny<0) {
                        nx = -nx;
                        ny = -ny;
                    }
                    //calculate distance of pi from the bond: dd = AP * n;
                    dd = abs(dx_pa * nx + dy_pa * ny);
                    //calculate force
                    if (dd<sij) {
                        
                        //calculate relative force
                        ftmp = (F[NDIM*gi] - (F[NDIM*gj] + F[NDIM*gk])/2) * nx + (F[NDIM*gi+1] - (F[NDIM*gj+1] + F[NDIM*gk+1])/2) * ny;
                        if (ftmp<0) {
                            ftmp -= 0.01;
                            fx = -ftmp * nx;
                            fy = -ftmp * ny;
                            
                            //set relative force to zero
                            F[NDIM*gi]             += fx*2/3;
                            F[NDIM*gi + 1]         += fy*2/3;
                        ...
                        F[NDIM*gk + 1]         -= fy/2;
                         */
                        // add to virial stress, what is dx and dy?
                        //stress[0]             += dx*fx;
                        //stress[1]             += dy*fy;
                        //stress[2]             += 0.5*(dx*fy + dy*fx);
                    }
...
void tumor2D::invasion(tumor2DMemFn forceCall, double dDr, double dPsi, double Drmin, int NT, int NPRINTSKIP){
	...
	for (k=0; k<NT; k++){
        ...
        //add vertice-bond repulsion to prevent penetration
        //verticeBondRepulsions();
