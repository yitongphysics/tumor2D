...
public:
    ...
    tumor2D(int n, int seed) : dpm(n,seed) { tN=n; gamtt=0.0; v0=0.0; Dr0=0.0; Ds=0.0; tau=0.0; rho0=0.0; rhot=0.0; kecm=0.0; ecmbreak=0.0; pbc[0]=0; pbc[1]=0; wpress.resize(2); };                     // tumor-only constructor
    tumor2D(int n, int tNval, int seed) : dpm(n,seed) { tN=tNval; gamtt=0.0; v0=0.0; Dr0=0.0; Ds=0.0; tau=0.0; rho0=0.0; rhot=0.0; kecm=0.0; ecmbreak=0.0; pbc[0]=0; pbc[1]=1; wpress.resize(2); };     // tumor+adipocyte constructor (x fixed, y periodic)
    ...
    void reNeighborLinkedList2D(double boxLengthScale);
    void neighborLinkedList2D();
    ...
    void tumorGrowth(double g0);
    void tumorDivide(double g0);
    ...
    double tumorRescale(double dt0);
    void invasion(tumor2DMemFn forceCall, double dDr, double dPsi, double Drmin, int NT, int NPRINTSKIP);
    void invasionConstP(tumor2DMemFn forceCall, double M, double P0, double g0, double dDr, double dPsi, double Drmin, int NT, int NPRINTSKIP);
