function vT_func(sim_name,kc,l1,l2)
%% voronoi tesselation
...
for ii = 1:1:FEND
    ...
    V_tw = [];% unique and ordered list of tumor touching wall
    V_ta = [];% unique and ordered list of tumor touching adipocytes
    C_tw = [];% unique and ordered list of tumor touching wall
    C_ta = [];% unique and ordered list of tumor touching adipocytes
            ...
            if(~isempty(find(r_vor{kk+nV}==jj,1)))% if this vertice belongs to tumor kk
                ...
                for ll=tN+1:nV
                    if(~isempty(find(r_vor{ll+nV}==jj,1))||...
                        ...
                        V_tumor = [V_tumor;jj];
                        if(V_vor(jj,1)>wpos && V_vor(jj,1)<Lx)
                            V_ta = [V_ta;jj];
                        end
    ...
    for kk=1:tN
        ...
        for jj=1:length(V_tw)
            if(~isempty(find(r_vor{kk+nV}==V_tw(jj),1)))% if this vertice belongs to tumor kk
                C_tw = [C_tw;kk];
                break
            end
        end
        
        for jj=1:length(V_ta)
            if(~isempty(find(r_vor{kk+nV}==V_ta(jj),1)))% if this vertice belongs to tumor kk
                C_ta = [C_ta;kk];
                break
            end
        end
    end
    N_sur(ii) = length(C_tumor);
    N_tw(ii) = length(C_tw);
    N_ta(ii) = length(C_ta);
    ...
    for jj=1:length(C_tumor)
        ...
        % u_ta
        r_a = r{tN+1}; r_a = r_a(1); % assuming vertices have the same size
        for kk=tN+1:nV
            sij = ri+r_a;
            xj=x_v(kk);
            yj=y_v(kk);
            ...
            if(dy<sij)
                ...
                if(dy<sij)
                    xij=rij/sij;
                    if(xij<1)
                        U_ta(ii) = U_ta(ii) + 0.5*(power(1.0 - xij,2.0));
                    end
                end
            end
        end
...
yourFolder = append(pathname,'vT');
if ~exist(yourFolder, 'dir')
   mkdir(yourFolder)
end
data_name = append(pathname,'vT/',sim_name,'_VT.mat');
save(data_name,'L_sur','U_tw','U_tt','U_ta','N_tw','N_ta','N_sur');
